...
export const ImportWallet: React.FC<ImportWalletProps> = ({ onImportWallet, onImportPrivateKey, loading }) => {
  ...
  const handleImportWallet = async () => {
    ...
    if (password && password.length < 8) {
      alert('Password must be at least 8 characters long (or leave empty for no password)');
      return;
    }
    ...
    if (mode === 'privateKey') {
      if (!secret.trim()) {
        alert('Please paste your secret key or recovery phrase');
        return;
      }
        ...
        {mode === 'privateKey' && (
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
              placeholder="Paste recovery phrase, Base58, hex, or JSON array (64 numbers)"
            />
            <p className="text-xs text-gray-500 mt-1">
              Supported: Recovery phrase (12 words), Base58, hex string, or JSON [..64 numbers..]
            </p>
          </div>
        )}
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder="Create a password for this wallet (optional)"
            />
            <button
              type="button"
